...
module.exports = function ({server, config}) {
    server.get('/users/:user.atom', (req, res) => {
        ...
        res.end(atomSchema.generate({
            ...
            id: `https://${config.HOST}/users/${req.params.user}.atom`,
            title: "WIP",
            updated: Date.now(),
            links: [
                { rel: "alternate", type: "text/html", href: `https://${config.HOST}/@${req.params.user}` },
                { rel: "self", type: "application/atom+xml", href: `https://${config.HOST}/users/${req.params.user}.atom` },
            ...
            author: {
            id: `https://${config.HOST}/users/${req.params.user}`,
            "activity:object-type": "http://activitystrea.ms/schema/1.0/person",
            uri: `https://${config.HOST}/users/${req.params.user}`,
            name: req.params.user,
            email: `${req.params.user}@${config.HOST}`,
            links: [
